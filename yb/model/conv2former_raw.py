...
# --- Baseline for CIFAR-100 (raw original arch, 224x224 input via upsampling) ---

@register_model
def conv2former_raw_s(pretrained=False, **kwargs):
    model = Conv2Former(kernel_size=11, dims=[72, 144, 288, 576], mlp_ratios=[4, 4, 4, 4],
                        depths=[4, 4, 32, 4], **kwargs)
    model.default_cfg = _cfg()
    return model
